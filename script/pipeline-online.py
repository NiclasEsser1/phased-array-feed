...
class Fold(Pipeline):
    ...
    def start(self, start_json, ra, dec):
        ...
        self._start_info = json.loads(start_json)
        self._utc_start_process = Time(self._start_info["utc_start_process"], format='isot', scale='utc')
        self._source_name = self._start_info["source_name"]
            ...
            for control_socket in self._input_control_socket:
                self._capture_control(control_socket,
                                      "START-OF-DATA_{}_{}_{}_{}".format(
                                          self._source_name, ra, dec, start_buf),
...
if __name__ == "__main__":
    ...
    config_info = {"utc_start_capture": Time(Time.now(), format='isot', scale='utc').value,
                   "frequency":         1340.5,
                   "ip_address":        ip,
                   "beam_el_d":         [0, -0.1, -0.2, -0.3, -0.1, -0.2, -0.3,  0.1,  0.2,  0.3, -0.11, -0.21, -0.31, -0.11, -0.21, -0.31,  0.12,  0.22,  0.32, -0.12, -0.22, -0.32, -0.12, -0.22, -0.32,  0.12,  0.22,  0.32, 0.13, 0.23,  0.33, -0.13, -0.23, -0.33, -0.13, -0.23, -0.33,  0.13,  0.23],
                   # First column in the file, opposite value                                                                                                  
                   "beam_xel_d":        [0, -0.1, -0.2, -0.3,  0.1,  0.2,  0.3, -0.1, -0.2, -0.3, -0.11, -0.21, -0.31,  0.11,  0.21,  0.31, -0.12, -0.22, -0.32, -0.12, -0.22, -0.32,  0.12,  0.22,  0.32, -0.12, -0.22, -0.32, -0.13, -0.23, -0.33, -0.13, -0.23, -0.33,  0.13,  0.23,  0.33, -0.13, -0.23],
                   # Second column in the file, opposite value
    }
    start_info = {"utc_start_process":  Time(Time.now(), format='isot', scale='utc').value,
    ...
    if pipeline == "fold":
        for i in range(nconfigure):
            ...
            for j in range(nstart):
                log.info("Start it ...")
                start_info["utc_start_process"] = Time(Time.now() + 10 * units.second, format='isot', scale='utc').value
                start_json = json.dumps(start_info)
                fold_mode.start(start_json, ra, dec)
                time.sleep(length)
                log.info("Stop it ...")
                fold_mode.stop()
    ...
    if pipeline == "search":
        for i in range(nconfigure):
            ...
            for j in range(nstart):
                ...
                start_info["utc_start_process"] = Time(Time.now() + 10 * units.second, format='isot', scale='utc').value
                start_json = json.dumps(start_info)
                search_mode.start(start_json, ra, dec)
    ...
    if pipeline == "spectrometer":
        for i in range(nconfigure):
            ...
            for j in range(nstart):
                ...
                start_info["utc_start_process"] = Time(Time.now() + 10 * units.second, format='isot', scale='utc').value
                start_json = json.dumps(start_info)
                spectrometer_mode.start(start_json, ra, dec)
                time.sleep(length)
                log.info("Stop it ...")
