...
#com_line = "docker run --ipc=host --cap-add=SYS_PTRACE --security-opt seccomp=unconfined -it --rm --runtime=nvidia -e DISPLAY --net=host -v {:s} -v {:s} -v /tmp:/tmp -u {:d}:{:d} -e NVIDIA_VISIBLE_DEVICES={:s} -e NVIDIA_DRIVER_CAPABILITIES=all --name {:s} xinpingdeng/{:s}".format(dvolume, hvolume, uid, gid, str(gpu), dname, dname)
