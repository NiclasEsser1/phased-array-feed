...
if __name__ == "__main__":
    ...
    fft_tsamp      = float(ConfigSectionMap(pipeline_conf, "BASEBAND2SPECTRAL")['fft_tsamp'])
    ...
    nsum = int(ndf_chk_stream * df_res / fft_tsamp)
    print nsum
    if (nsum < (2 * BLKSZ)):
        twice_sum = 0
        sum1_blksz = nsum / 2
    ...
    # Do the work
    #b2p = "../src/baseband2spectral/baseband2spectral_main -a {:s} -b {:s} -c {:d} -d {:d} -e {:d} -f {:d} -g {:d} -i {:d} -j {:s}".format(key_input, key_output, ndf_chk_rbuf, nrepeat, nstream, ndf_chk_stream, twice_sum, sum1_blksz, ddir)
    b2p = "nvprof ../src/baseband2spectral/baseband2spectral_main -a {:s} -b {:s} -c {:d} -d {:d} -e {:d} -f {:d} -g {:d} -i {:d} -j {:s}".format(key_input, key_output, ndf_chk_rbuf, nrepeat, nstream, ndf_chk_stream, twice_sum, sum1_blksz, ddir)
    #b2p = "cuda-memcheck ../src/baseband2spectral/baseband2spectral_main -a {:s} -b {:s} -c {:d} -d {:d} -e {:d} -f {:d} -g {:d} -i {:d} -j {:s}".format(key_input, key_output, ndf_chk_rbuf, nrepeat, nstream, ndf_chk_stream, twice_sum, sum1_blksz, ddir)
