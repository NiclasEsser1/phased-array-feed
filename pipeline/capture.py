...
#    exit()
    ...
    if (len(destination_dead) == 0):
        capture_command = "../src/capture/capture_main -a {:s} -b {:d} -c {:d} -d {:s} -f {:f} -g {:d} -i {:d} -j {:d} -k {:s} -l {:d} -m {:d} -n {:d} -o {:d} -p {:d} -q {:d} -r {:d} -s {:d} -t /tmp/capture.beam{:d}.part{:d}".format(key, pktsz, pktoff, " -d ".join(destination_active), freqs[beam][part], nchans[beam][part], refinfo[0], refinfo[1], dir_capture, cpu + 1, cpu + 2, bind, sec_prd, nchunk, ndf_chk_rbuf, ndf_chk_tbuf, ndf_chk_prd, beam, part)
    else:
        capture_command = "../src/capture/capture_main -a {:s} -b {:d} -c {:d} -d {:s} -e {:s} -f {:f} -g {:d} -i {:d} -j {:d} -k {:s} -l {:d} -m {:d} -n {:d} -o {:d} -p {:d} -q {:d} -r {:d} -s {:d} -t /tmp/capture.beam{:d}.part{:d}".format(key, pktsz, pktoff, " -d ".join(destination_active), " -e ".join(destination_dead[beam][part]), freqs[beam][part], nchans[beam][part], refinfo[0], refinfo[1], dir_capture, cpu + 1, cpu + 2, bind, sec_prd, nchunk, ndf_chk_rbuf, ndf_chk_tbuf, ndf_chk_prd, beam, part)
