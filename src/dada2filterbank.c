...
int filterbank_header(conf_t conf)
{
  ...
  /* Write file name */
  write_string(conf.f_fp, "rawdatafile");
  write_string(conf.f_fp, conf.f_fname);

  /* Write telescope_id */
  write_int(conf.f_fp, "telescope_id", conf.telescope_id);
  ...
  /* Write tstart */
  write_double(conf.f_fp, "tstart", conf.mjd_start);

  /* Write nbits */
  write_int(conf.f_fp, "nbits", conf.nbits);
  
  /* Write nifs */
  write_int(conf.f_fp, "nifs", conf.nifs);

  /* Write fch1 */
  write_double(conf.f_fp, "fch1", conf.fch1);
  
  /* Write foff */
  write_double(conf.f_fp, "foff", conf.foff);

  /* Write raj */
  write_double(conf.f_fp, "src_raj", conf.raj);

  /* Write dej */
  write_double(conf.f_fp, "src_dej", conf.dej);

  /* Write nchans */
  write_int(conf.f_fp, "nchans", conf.nchans);

  /* Write source name */
  write_string(conf.f_fp, "source_name");
  write_string(conf.f_fp, conf.source_name);

  /* write machine_id */
  write_int(conf.f_fp, "machine_id", conf.machine_id);
  
  write_string(conf.f_fp, "HEADER_END");
...
int dada_header(conf_t *conf)
{
  ...
  ascii_header_get(hdrbuf, "DEC", "%s", conf->de);
  ascii_header_get(hdrbuf, "MJD_START", "%lf", &conf->mjd_start);
  ascii_header_get(hdrbuf, "PICOSECONDS", "%"SCNu64"", &conf->picoseconds);
  ...
  fprintf(stdout, "conf->ra is %s\n", conf->ra);
  fprintf(stdout, "conf->de is %s\n", conf->de);
  conf->raj = hms_degree(conf->ra);
  conf->dej = dms_degree(conf->de); // Sigproc angle to hms is not right, it converts angle to dms for both ra an dec
