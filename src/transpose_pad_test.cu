...
// ./transpose_pad_test -a 48 -b 1024 -c 64
// ./transpose_pad_test -a 33 -b 1024 -c 64

extern "C" void usage ()
{
  fprintf (stdout,
	   "transpose_pad_test - Test the transpose_pad kernel \n"
	   "\n"
	   "Usage: transpose_pad_test [options]\n"
...
int main(int argc, char *argv[])
{
	  ...
	  for(k = 0; k < block_size.x; k++)
	    {
	      ...
	      h_result[NPOL_BASEBAND * idx_out].x   = data[idx_in].x + data[idx_in].y;
	      h_result[NPOL_BASEBAND * idx_out].y   = data[idx_in].x * data[idx_in].x + data[idx_in].y * data[idx_in].y;
	      h_result[NPOL_BASEBAND * idx_out+1].x = data[idx_in+nsamp_in].x + data[idx_in+nsamp_in].y;
	      h_result[NPOL_BASEBAND * idx_out+1].y = data[idx_in+nsamp_in].x * data[idx_in+nsamp_in].x + data[idx_in+nsamp_in].y * data[idx_in+nsamp_in].y;
	    }
	}
    }
  ...
  transpose_pad_kernel<<<grid_size, block_size>>>(g_in, nsamp_in, g_out);
