...
void *do_capture(void *conf)
{
  ...
  double freq;
  char *dbuf = NULL;
  int sock, chunk_index;	// TODO Maybe beam index?!
  int64_t df_in_blk;
  ...
  register int nbeam = capture_conf->nbeam;
	register int beam_index = 0;
...
int initialize_capture(conf_t *conf)
{
  ...
  for(i = 0; i < conf->nport_alive; i++)
    {
      conf->nbeam        += conf->nbeam_alive_expect_on_port[i];
      conf->nbeam_alive  += conf->nbeam_alive_actual_on_port[i];
    }
  for(i = 0; i < conf->nport_dead; i++)
    conf->nbeam       += conf->nbeam_dead_on_port[i];
  if(conf->pad == 1)
    conf->nbeam = NCHUNK_FULL_BEAM;
  if(conf->nbeam_alive == 0)
    {
  ...
  // conf->nchan        = conf->nbeam * NCHAN_PER_CHUNK;		// TODO: Maybe conf->nbeam = NCHAN_PER_CHUNK
	// Added by Niclas
	conf->nchan				= NCHAN_PER_CHUNK;
	// Commented by Niclas
  // conf->chunk_index0 = -(conf->center_freq + 1.0)/(double)NCHAN_PER_CHUNK + 0.5 * (conf->nbeam + 1);
  ...
  conf->blksz_rbuf = conf->nbeam * conf->dfsz_keep * conf->ndf_per_chunk_rbuf;  // The required ring buffer block size in byte;
  ...
  conf->tbufsz = (conf->dfsz_keep + 1) * conf->ndf_per_chunk_tbuf * conf->nbeam;
...
int register_dada_header(conf_t conf)
{
  ...
  // bytes_per_second_out = (uint64_t)(bytes_per_second_template * conf.nchan/(double)nchan_template);
	// Changed by Niclas
	bytes_per_second_out = bytes_per_second_template;
  ...
  file_size_out = (uint64_t)(file_size_template);
...
{
  conf_t *capture_conf = (conf_t *)conf;
  int i, chunk_index, idf, transited = 0, nbeam = capture_conf->nbeam;
  ...
  while(!quit)
	...
	If all ports have packet arrive after current ring buffer block, start to change the block
      */
      ...
      // if(ndf_actual >= capture_conf->total_data_frames && !(capture_conf->total_data_frames == -1))
      // {
      //   printf("NDF: (%d/%d)",ndf_actual,capture_conf->total_data_frames );
...
int examine_record_arguments(conf_t conf, char **argv, int argc)
{
  ...
  for(i = 0; i < conf.nport_alive; i++)
    log_add(conf.log_file, "INFO", 1,  "    ip %s, port %d, expected beam chunks %d and actual beam chunks %d", conf.ip_alive[i], conf.port_alive[i], conf.nbeam_alive_expect_on_port[i], conf.nbeam_alive_actual_on_port[i]);
  ...
  if(conf.nport_dead == 0)  // Does not matter how many dead ports we have
  ...
  else
    {
      ...
      for(i = 0; i < conf.nport_dead; i++)
	log_add(conf.log_file, "WARN", 1,  "    ip %s, port %d, expected beam chunks %d", conf.ip_dead[i], conf.port_dead[i], conf.nbeam_dead_on_port[i]);
    }
  ...
  for(i = 0; i < NPORT_MAX; i++)
    {
      ...
      conf->nbeam_dead_on_port[i] = 0;
      conf->nbeam_alive_expect_on_port[i] = 0;
      conf->nbeam_alive_actual_on_port[i] = 0;
    }
  return EXIT_SUCCESS;
}
