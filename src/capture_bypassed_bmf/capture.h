...
typedef struct conf_t
{
  ...
  int nbeam_alive_expect_on_port[NPORT_MAX], nbeam_alive_actual_on_port[NPORT_MAX], nbeam_dead_on_port[NPORT_MAX];;  // For each port;
  int nchan, nchunk, nbeam_alive;

  // Added by Niclas
  int nbeam, chan_index;

  char obs_id[MSTR_LEN];

  char dir[MSTR_LEN], dada_header_template[MSTR_LEN], fname_log[MSTR_LEN];
