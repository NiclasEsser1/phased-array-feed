...
__global__ void mean_kernel(cufftComplex *buf_rt1, uint64_t offset_rt1, float *ddat_offs, float *dsquare_mean, int nstream, float scl_ndim); // Share between fold and search mode
...
__global__ void detect_add_scale_kernel(cufftComplex *dbuf_rt1, uint8_t *dbuf_out, uint64_t offset_rt1, float *ddat_offs, float *ddat_scl);
__global__ void detect_add_pad_transpose_kernel(cufftComplex *dbuf_rt1, cufftComplex *dbuf_rt2, uint64_t offset_rt1);
#endif