...
void *buf_control(void *conf)
{
  ...
  sleep_time   = 0.9 * captureconf->rbuf_blk_sec; // Sleep for part of buffer block time to save cpu source;
      ...
      if(transited)                   // Get new buffer when we see transit
	{
	  ...
	  while(!quit) // Wait until all threads are on new buffer block
	      ...
	      if(transited == 0) 
		{
		  ...
		  for(i = 0; i < captureconf->nport_active; i++)
		    {
		      ...
		      ndf_port_expect = captureconf->rbuf_ndf_chk * captureconf->nchk_active_actual[i]; // Only for current buffer
		      
		      fprintf(stdout, "Thread %d:\t\t%E\t%"PRIu64"\t%"PRIu64"\t%.1E\n", i, rbuf_iblk * captureconf->rbuf_blk_sec, ndf_port_actual, ndf_port_expect, 1.0 - ndf_port_actual/(double)ndf_port_expect);
		      loss_rate += fabs((1.0 - ndf_port_actual/(double)ndf_port_expect)/(double)captureconf->nport_active);
		    }
		  fprintf(stdout, "\n");
		  ...
		  /* Sleep half block time to save cpu source */
		  sleep(sleep_sec);
		  usleep(sleep_usec);
...
{  
  ...
  sec_ref = floor(captureconf->sec_prd) + captureconf->sec_ref + SECDAY * captureconf->epoch_ref;
  picoseconds_ref = 1E6 * round(1.0E6 * (captureconf->sec_prd - floor(captureconf->sec_prd)));
