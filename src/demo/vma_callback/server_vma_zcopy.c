...
do_read(int fd, struct vma_api_t *vma_api)
{
	...
	if (ret > 0) {
		if ((flags & MSG_VMA_ZCOPY) == MSG_VMA_ZCOPY) {
			...
			if (datalen != ret) {
				fprintf(stderr, "## ERROR: Datalen from vma_packet's iovec: %d != "
					"Return value from recvfrom_zcopy: %d !!!\n",
					datalen, ret);
			}
	...
	printf("Total Data Recieved [socketfd: %d]: Based on iovec: %d bytes,  "
			"Based on return value: %d bytes\n", fd, total_iovec_datalen, total_ret_datalen);
	free(buf);
}
